...
def create_excel_gantt(df_tasks, output_file):
    ...
    summary_headers = ["Resource", "Driver", "Location", "Tasks", "Task Count", "Total Duration (months)", "Months"]
    ...
    for i, (_, row) in enumerate(resource_summary.iterrows(), 2):
        ...
        ws_summary.cell(row=i, column=5).value = row['Task_Count']
        ws_summary.cell(row=i, column=6).value = row['Total_Duration']
        ws_summary.cell(row=i, column=7).value = row['Months']
